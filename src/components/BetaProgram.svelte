...
<section
        class="max-w-4xl mx-auto px-4 py-8 my-10 rounded-2xl relative overflow-hidden"
...
>
    ...
    >
        We're selecting <span class="text-purple-300 font-semibold">{betaTestersInfo.total}</span> students for our
        exclusive beta program. Approximately <span
            class="text-purple-300 font-semibold">{betaTestersInfo.perSkill}</span> students per course category will
        get early access to test our platform.
    </p>

    <!-- Benefits and How to Apply Grid -->
    <div class="grid md:grid-cols-2 gap-8 mt-20 mb-8">
        <!-- Benefits Card -->
        <div
        ...
        >
            ...
            <ul class="space-y-3">
                {#each betaTestersInfo.benefits as benefit, i}
                    <li
                            class="flex items-start justify-start text-gray-300"
                            in:fade={{ duration: 500, delay: 400 + i * 100 }}
                    >
                        <svg class="w-5 h-5 text-purple-400 mr-2 flex-shrink-0" fill="none" stroke="currentColor"
                             viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                        </svg>
                        <span class="text-start">{benefit}</span>
                    </li>
                {/each}
            </ul>
        </div>
